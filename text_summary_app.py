...
def index():
    ...
    if request.method == "POST":
        ...
        summary_length_option = request.form.get("length")

        # Map user selection to number of sentences
        length_map = {"short": 2, "medium": 4, "long": 6}
        summary_length = length_map.get(summary_length_option, 3)

        if text:
            summary = summarize_text(text, summary_length=summary_length)
